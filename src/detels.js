...
const Movies = () => {
  ...
  useEffect(() => {
    ...
    fetch(`/.netlify/functions/tmdb?path=${encodeURIComponent(`/movie/${id}?language=${apiLang}`)}`)
      .then(async (res) => {
        if (!res.ok) {
          if (res.status === 404) {
            throw new Error("404");
          ...
          if (needsOverview || needsBackdrop || needsTitle) {
            try {
              const fallbackRes = await fetch(
                `/.netlify/functions/tmdb?path=${encodeURIComponent(`/movie/${id}?language=en-US`)}`
              );
              if (fallbackRes.ok) {
    ...
    // Fetch credits (cast)
    fetch(`/.netlify/functions/tmdb?path=${encodeURIComponent(`/movie/${id}/credits?language=${apiLang}`)}`)
      .then((res) => {
        ...
        if (allVideos.length === 0) {
          try {
            const globalRes = await fetch(
              `/.netlify/functions/tmdb?path=${encodeURIComponent(`/movie/${id}/videos`)}`
            );
            if (globalRes.ok) {
    ...
    fetch(`/.netlify/functions/tmdb?path=${encodeURIComponent(`/movie/${id}/similar?language=${apiLang}`)}`)
      .then((res) => {
        if (res.ok) return res.json();
        return null;
      })
    ...
    fetch(`/.netlify/functions/tmdb?path=${encodeURIComponent(`/movie/${id}/reviews`)}`)
      .then((res) => {
        if (res.ok) return res.json();
        return null;
      })
