...
export default function Hero({
  ...
  const handleAskAI = async () => {
    ...
    if (!GEMINI_API_KEY) {
      setMessages((prev) => [
        ...
        {
          text: isInputArabic
            ? "خطأ: لم يتم العثور على مفتاح الـ API الخاص بـ OpenRouter."
    ...
    const currentLang = isInputArabic ? "ar" : "en";

    const systemPrompt = `You are a highly intelligent movie expert. Analyze the user's current context/mood and recommend 3 to 5 matching movies or TV shows. For each, give the title and a 2-line description. Answer fully in Arabic if the user prompt is Arabic, and fully in English if English.`;
    ...
    try {
      ...
      if (data.choices?.[0]?.message?.content) {
        ...
        setMessages((prev) => [
          ...
          {
            text: currentLang === "ar"
              ? "لم أتمكن من معالجة النص، يرجى المحاولة مرة أخرى."
  ...
  const suggestions  = isInputArabic ? SUGGESTIONS_AR : SUGGESTIONS_EN;
  const dir          = isInputArabic ? "rtl" : "ltr";

  return (
    <div ref={containerRef} className="ai-hero">
      ...
      <div className="ai-card">
        ...
        {/* Green accent stripe at top — matches .section-title / .tagline border colour */}
        ...
        <div className="ai-card__header">
          ...
          <div className="ai-header-text">
            <p className="ai-header-title">
              {isInputArabic ? "مساعد الأفلام الذكي" : "AI Movie Assistant"}
            </p>
            <p className="ai-header-sub">
              {isInputArabic
                ? "أخبرني عن مزاجك، وسأختار لك السهرة المثالية"
                : "Tell me your mood — I'll pick your perfect watch"}
            </p>
          </div>
          {/* Online indicator */}
        ...
        <div ref={messagesRef} className="ai-messages">
          ...
          {!hasMessages && (
            <div className="ai-welcome">
              <span className="ai-welcome__emoji">🍿</span>
              <p className="ai-welcome__title">
                {isInputArabic ? "مرحباً! كيف حال يومك؟" : "What's your vibe tonight?"}
              </p>
              <p className="ai-welcome__text">
                {isInputArabic
                  ? "احكِ لي كيف كان يومك أو ما المزاج الذي أنت عليه، وسأرشح لك أفضل ما يناسبك."
                  : "Describe your day or mood and I'll find the perfect film or show for your evening."}
              </p>
            </div>
          )}
        ...
        <div className="ai-input-area" dir={dir}>
          <textarea
            ...
            placeholder={
              isInputArabic
                ? "احكِ لي عن يومك أو مزاجك..."
                : "Describe your day or mood… (Ctrl+Enter to send)"
            }
            disabled={isLoading}
            rows={1}
          />
